            ...
            <?php
                ...
                if ($_POST['mode'] == "dates"){
                    ...
                    while ($devoir = $q -> fetch()){
                        ...
                        echo '<input type="hidden" name="matiere" id="matiere" value="'. $_POST['matiere'] .'">';
                        echo '<input type="hidden" name="emoji" id="emoji" value="'. $_POST['emoji'] .'">';
                        echo '<a href="#lol" style="background-color : #152d4d" onclick="this.parentNode.submit()"> '. $devoir['date'] .' </a>';
                        echo '</form>';
                    }
                }
                else if ($_POST['mode'] == "contenus"){
                    echo '<h2>Lisez le devoir</h2>';
                }
            ?>
        </div>
        ...
        <div class="droite" style="background-color: #28548e">
            <?php
                if ($_POST['mode'] == "dates"){
                    ...
                    $date = $_POST['date'];
                    $q = $db -> query("SELECT * FROM recreation_devoirs_contenus WHERE matiere = '$matiere' AND date = '$date'");
